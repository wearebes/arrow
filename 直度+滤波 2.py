        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if drawing:
                ...
                if is_near_center(x, y, center):
                    cv2.circle(temp_frame, center, 15, (255, 255, 0), 1)  # 高亮圆心区域
...
def get_scale_reference(frame):
    ...
    display_frame = frame.copy()
    h, w = frame.shape[:2]
    dragging_point_idx = None  # 正在拖动的端点索引（0或1）
    dragging_midpoint = False  # 是否正在拖动中点
    initial_click_pos = None
    initial_points = None
